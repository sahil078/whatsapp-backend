...
export function makeWebhookController(io?: SocketIOServer) {
  return {
    simulateWebhook: async (req: Request, res: Response) => {
      ...
      for (const item of items) {
        if (item.kind === "message") {
          ...
          if (doc) io?.to(doc.wa_id).emit("message:status", { meta_msg_id: doc.meta_msg_id, msg_id: doc.msg_id, status: doc.status });
        }
      }
